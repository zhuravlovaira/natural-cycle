...
export class FitTextInParentHorizontalDirective implements OnInit, OnDestroy {
  ...
  constructor(
   ...
   private readonly reactOnParentSizeChange = () => {
    ...
    const parentElementWidth: number | undefined = this.elementRef.nativeElement.parentElement?.offsetWidth;
    ...
    const newFontSize: number = scaledFontSize >= initialFontSize ? initialFontSize : scaledFontSize;
  ...
  private getFontSize(): number {
    const fontSize: string = window.getComputedStyle(this.elementRef.nativeElement).fontSize;
